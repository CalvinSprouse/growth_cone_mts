...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/model/sim_default.pdf}
    \caption{\label{fig:sim_steady}
        Note for feedback, figure not in final form (black background especially needs to go).

        Representative output of the steady state of the actomyosin network from Ref.~\cite{craig2012bj}. The pink line represents the retrograde actin flow speed from the leading edge, \(x=\qty{0}{\micro\meter}\), to the trailing edge, \(x=\qty{12}{\micro\meter}\).
    }
\end{figure}

In addition to the features of this model we introduce MTs protruding from the axon via Dynein sliding forces. After the network has reached a steady state shown in Figure~\ref{fig:sim_steady} we permit MTs to enter the actomyosin network through Dynein sliding forces and MT polymerization. Here we introduce two distinct behaviors for testing:
\begin{enumerate}
